...
class PreviewEngine(Engine):
    ...
    def separate(self, audio, mode, progress=lambda p, s: None):
        ...
        # "other"/"guitar"/"piano" = whatever remains after the above. The DSP
        # can't actually tell them apart, so each one is an equal slice of the
        # same residual signal.
        residual = audio - (vocals + bass + drums)
        residual = residual.astype(np.float32)

        # Share the residual evenly across the *distinct part names* referenced
        # anywhere in this mode's groups (not across output channels): a channel
        # like 5stems' "other": ["other", "guitar"] references two residual
        # parts, so it must get two shares, or its output ends up louder than a
        # channel referencing only one part (e.g. "piano": ["piano"]) even
        # though both are equally arbitrary residual buckets. Counting channels
        # instead of parts under-counts that case and inflates total energy
        # past the actual residual signal.
        spec = config.STEM_MODES[mode]
        residual_parts = {p for parts in spec["groups"].values() for p in parts
                          if p in ("other", "guitar", "piano")}
        share = 1.0 / len(residual_parts) if residual_parts else 0.0
        for extra in residual_parts:
